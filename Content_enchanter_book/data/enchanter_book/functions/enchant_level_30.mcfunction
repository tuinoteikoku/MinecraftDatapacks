### Copyright © 2022 よしたこ
### エンチャントレベル30相当のエンチャントを実行するfunction
### 短い処理なのでべた書きしています
...
#レベルが特定の数値以上であることを確認
execute if entity @s[level=3..] run tag @s add eb_book

#ラピスラズリを特定数以上所有していることの確認
execute store result score @s Enchanter_book.lapis_num run clear @s minecraft:lapis_lazuli 0
execute if score @s Enchanter_book.lapis_num matches 3.. run tag @s add eb_lps
...
#使用できる場合の処理
execute if entity @s[tag=eb_lv,tag=eb_book,tag=eb_lps] run loot give @s loot enchanter_book:enchant_level_30
execute if entity @s[tag=eb_lv,tag=eb_book,tag=eb_lps] run xp add @s -3 levels
execute if entity @s[tag=eb_lv,tag=eb_book,tag=eb_lps] run clear @s lapis_lazuli 3
